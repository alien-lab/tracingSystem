        ...
        function($scope,$rootScope,loginService,$state,$cookieStore){
            ...
            $scope.dologin=function(){
                ...
                loginService.dologin({
                    account:$scope.login.loginname,
                    password:$scope.login.password
                },function(result){
                    console.log("yanzheng"+result.result);
                    if(result.result >0 ){//登录成功
                        console.log("不打印就拿捏不了"+result.data.account);
                        console.log("登陆成功")
                       $rootScope.user=result.data;
                       $state.go("lvzhiyuan.batchQuery");//登录成功跳转到主页
                        // Put cookie
...
(function() {
    ...
    angular.module("app.core").service("loginService",["$http",function($http){
        ...
        this.dologin=function(user1,callback){
            console.log(user1.account+user1.password);
            $http({
                url:"/user1/dologin",
                method:"POST",
                data:{
